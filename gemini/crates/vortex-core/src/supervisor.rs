...
use crate::shm::{WorkerStatus, SharedMemory};
...
impl Supervisor {
    ...
    pub fn new(worker_script: impl Into<String>, shm: Arc<SharedMemory>) -> Self {
        Self {
...
impl SupervisorTrait for Supervisor {
    ...
    fn check_health(&self, pid: i32) -> HealthStatus {
        match self.workers.get(&pid) {
            Some(worker) => {
                // Check if worker is booting
                if worker.status == WorkerStatus::Booting {
                    let elapsed = worker.spawn_time.elapsed().as_millis() as u64;
                    if elapsed < 10000 { // 10s grace period for boot
                        return HealthStatus::Healthy;
                    }
                }

                // Check heartbeat in SHM
                if let Some(slot_id) = self.shm.find_slot_by_pid(pid as u32) {
                    if let Some(slot) = self.shm.slot(slot_id) {
                        let current_tick = self.shm.header().clock();
                        // 1 tick = 1ms, heartbeat_timeout is in ms
                        if slot.is_alive(current_tick, self.heartbeat_timeout) {
                            return HealthStatus::Healthy;
                        } else {
                            tracing::warn!(pid = %pid, "Worker heartbeat timeout");
...
mod tests {
    ...
    fn test_supervisor_creation() {
        ...
        let shm = Arc::new(SharedMemory {
            base: buffer.as_mut_ptr(),
            size: 1024,
            fd: -1,
        });

        let sup = Supervisor::new("worker.py", shm)
            .with_max_workers(8)
